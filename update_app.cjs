...
// 4. Modifying finishQuiz payload
content = content.replace(
  "score: score,",
  `score: score,
...
// 5. Modifying loginAdmin
const loginAdminNew = `const loginAdmin = async () => {
    try {
...
content = content.replace(/const loginAdmin = async \(\) => \{[\s\S]*?\n  \};/, loginAdminNew);

// 6. Functions for Question Management
const adminQuestionFcns = `
...
`;
content = content.replace("const handleWhatsAppShare = () => {", adminQuestionFcns + "\n  const handleWhatsAppShare = () => {");

// 7. Modifying admin-dashboard UI UI completely
const dashboardRegex = /\{view === 'admin-dashboard' && \([\s\S]*?\}\)/;
